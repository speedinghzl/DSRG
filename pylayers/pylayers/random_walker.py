...
def _compute_weights_3d(data, unary, spacing, alpha=130, beta=130, eps=1.e-6,
                        multichannel=False):
    ...
    gradients_unary = 0
    for channel in range(0, unary.shape[-1]):
        gradients_unary += _compute_gradients_3d(unary[..., channel],
                                           spacing) ** 2
    # All channels considered together in this standard deviation
    alpha /= 10 * unary.std()
    if multichannel:
        # New final term in beta to give == results in trivial case where
        # multiple identical spectra are passed.
        alpha /= np.sqrt(unary.shape[-1])
    gradients_unary *= alpha

    # print np.mean(gradients), np.mean(gradients_unary)
    weights = np.exp((- gradients - gradients_unary))
    weights += eps
    return weights
...
def _build_laplacian(data, unary, spacing, mask=None, alpha=50, beta=50,
                     multichannel=False):
    l_x, l_y, l_z = tuple(data.shape[i] for i in range(3))
    edges = _make_graph_edges_3d(l_x, l_y, l_z)
    weights = _compute_weights_3d(data, unary, spacing, alpha=alpha, beta=beta, eps=1.e-10,
...
def random_walker(data, unary, labels, alpha=130, beta=130, mode='bf', tol=1.e-3, copy=True,
    ...
    if np.any(labels < 0):
        lap_sparse = _build_laplacian(data, unary, spacing, mask=labels >= 0,
                                      alpha=alpha, beta=beta, multichannel=multichannel)
    else:
        lap_sparse = _build_laplacian(data, unary, spacing, alpha=alpha, beta=beta,
                                      multichannel=multichannel)
